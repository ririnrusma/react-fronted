...
export default function Card({product}) {
  return (
    <Link to={`/products/${product.slug}` ?? ''} className="flex flex-col max-w-[370px] flex-wrap p-[16px] bg-white hover:ring-opacity-40 active:ring-5 active:ring-[#6247eb] hover:ring-4 active:ring-2 active:ring-opacity-90 rounded-lg" >
    <div className="flex flex-col max-w-[370px] flex-wrap p-[16px] bg-white" >
          <img src={product.imageUrl ?? ''} alt={product.name ?? 'No name'} className="block max-h-[300px] mb-4 object-cover rounded-lg" />
            <div className="flex flex-col gap-2">
                <h4 className="font-medium text-[20px] text-black">{product.name ?? 'No Name'}</h4>
                <span className="block font-medium text-[14px] text-gray">{product.category ?? 'Uncatagorized'}</span>
